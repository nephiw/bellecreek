...
export class MeetingDateService {
  ...
  private getThirdWednesdayOfMonth(date: Date) {
  ...
  public getNextMeeting(): Observable<string> {
    const today = new Date();
    let month: string;
    let year = today.getFullYear();
    let day = this.getThirdWednesdayOfMonth(today);
    ...
    this.nextMeeting$ = new BehaviorSubject(`${month} ${day}, ${year}`);
